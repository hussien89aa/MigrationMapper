...
CREATE TABLE  MigrationMapping  (
	 ...
	 FromCode	TEXT,
	 ToCode 	TEXT,
	MigrationMappingID	INTEGER PRIMARY KEY AUTO_INCREMENT
);
describe MigrationMapping; 


 
CREATE TABLE MappingConfident  (
	 MigrationMappingID 	INTEGER,
	 ConfidentTypeID 	INTEGER,
	 ConfidentValue 	REAL,
	 isVaildMapping INTEGER DEFAULT 0

);
describe MappingConfident; 

 
CREATE TABLE  ConfidentType  (
	 ConfidentTypeID 	INTEGER,
	 ConfidentTypeName 	varchar(50)
);
describe ConfidentType; 

insert into ConfidentType(ConfidentTypeID,ConfidentTypeName) values (0,'Substitution');
select * from ConfidentType;
